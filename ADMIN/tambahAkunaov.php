...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $koneksi->prepare("SELECT preview_image FROM akun WHERE id = ?");
...
if (isset($_POST['edit_data'])) {
    ...
    if (!empty($_FILES["preview_image"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["preview_image"]["tmp_name"], $targetFile)) {
            ...
            $stmt = $koneksi->prepare("UPDATE akun SET penjual=?, skin=?, level=?, hero=?, price=?, deskripsi=?, preview_image=?, nomor_wa=? WHERE id=?");
            $stmt->bind_param("siisisssi", $penjual, $skin, $level, $hero, $price, $deskripsi, $targetFile, $nomorWa, $id);
        }
    } else {
        $stmt = $koneksi->prepare("UPDATE akun SET penjual=?, skin=?, level=?, hero=?, price=?, deskripsi=?, nomor_wa=? WHERE id=?");
...
<head>
    ...
    <title>Tambah akun Aov</title>
    <link rel="stylesheet" href="styletambahakun.css">
</head>
<body style="background-image: url('../image/aov.jpeg'); background-size: cover; background-repeat: no-repeat;">
    ...
    <h1>Tambah Akun</h1>
        ...
        <button type="submit" name="add_data">Tambah Produk</button>
        <table border="1">
        <thead>
        <tr>
                <th>Penjual</th>
    ...
    if (isset($_GET['edit_id'])) {
        $editId = $_GET['edit_id'];
        $stmt = $koneksi->prepare("SELECT * FROM akun WHERE id = ?");
