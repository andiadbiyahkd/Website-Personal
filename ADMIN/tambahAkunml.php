...
if (isset($_POST['add_data'])) {
    ...
    if (move_uploaded_file($_FILES["preview_image"]["tmp_name"], $targetFile)) {
        ...
        $stmt = $koneksi->prepare("INSERT INTO akunml (penjual, skin, level, hero, price, deskripsi, preview_image, nomor_wa) 
                                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt->execute()) {
            ...
            redirectWithMessage("error_message", "Gagal menambahkan produk");
        }
        $stmt->close();
    } else {
...
if (isset($_GET['delete_id'])) {
    ...
    $stmt = $koneksi->prepare("SELECT preview_image FROM akunml WHERE id = ?");
    ...
    $stmt = $koneksi->prepare("DELETE FROM akunml WHERE id = ?");
    $stmt->bind_param("i", $deleteId);
...
if (isset($_POST['edit_data'])) {
    ...
    if (!empty($_FILES["preview_image"]["name"])) {
        ...
        if (move_uploaded_file($_FILES["preview_image"]["tmp_name"], $targetFile)) {
            ...
            $stmt = $koneksi->prepare("UPDATE akunml SET penjual=?, skin=?, level=?, hero=?, price=?, deskripsi=?, preview_image=?, nomor_wa=? WHERE id=?");
            $stmt->bind_param("siisisssi", $penjual, $skin, $level, $hero, $price, $deskripsi, $targetFile, $nomorWa, $id);
        }
    } else {
        $stmt = $koneksi->prepare("UPDATE akunml SET penjual=?, skin=?, level=?, hero=?, price=?, deskripsi=?, nomor_wa=? WHERE id=?");
...
<head>
    ...
    <title>Tambah akun Ml</title>
    <link rel="stylesheet" href="styletambahakun.css">
</head>
<body style="background-image: url('../image/Benedetta.jpeg'); background-size: cover; background-repeat: no-repeat;">
    ...
    <h1>Tambah Produk</h1>
        ...
        <input type="text" name="hero" placeholder="Hero Utama" required><br>
        ...
        <tbody>
            <?php
            $result = $koneksi->query("SELECT * FROM akunml");
    ...
    if (isset($_GET['edit_id'])) {
        $editId = $_GET['edit_id'];
        $stmt = $koneksi->prepare("SELECT * FROM akunml WHERE id = ?");
